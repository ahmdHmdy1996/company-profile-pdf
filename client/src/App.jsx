...
import Creative from "./templates/Creative.jsx";
...
const templates = {
  ...
  creative: { name: "Creative", component: Creative },
  minimal: { name: "Minimal", component: Minimal },
};
...
export default function App() {
  ...
  const addApproval = () => {
    const newApproval = "";
    update("approvals", [...(data.approvals || []), newApproval]);
  };

  const removeApproval = (index) => {
    const newApprovals = (data.approvals || []).filter((_, i) => i !== index);
  ...
  const removeClient = (index) => {
    const newClients = (data.clients || []).filter((_, i) => i !== index);
    update("clients", newClients);
  };
              ...
              >
                <InputField
                  ...
                  options={[
                    ...
                    {
                      value: "creative",
                      label: data.rtl ? "إبداعي (ملون)" : "Creative (Colorful)",
                    },
                    {
                      value: "minimal",
                      label: data.rtl ? "بسيط (أنيق)" : "Minimal (Elegant)",
                    },
                  ]}
                  rtl={data.rtl}
                />
                ...
                {(data.services || []).map((service, index) => (
                  ...
                  >
                    ...
                    <InputField
                      ...
                      value={service}
                      onChange={(e) =>
                        update(`services.${index}`, e.target.value)
                      }
                      placeholder={data.rtl ? "اسم الخدمة" : "Service name"}
                      rtl={data.rtl}
                    />
                  </div>
                ))}
                <button onClick={addService} className="w-full btn-secondary">
                  {data.rtl ? "إضافة خدمة" : "Add Service"}
                </button>
              </FormSection>
            </div>
                ...
                {(data.clients || []).map((client, index) => (
                  ...
                  >
                    ...
                    <InputField
                      ...
                      value={client}
                      onChange={(e) =>
                        update(`clients.${index}`, e.target.value)
                      }
                      placeholder={data.rtl ? "اسم العميل" : "Client name"}
                      rtl={data.rtl}
                    />
                  </div>
                ))}
                <button onClick={addClient} className="w-full btn-secondary">
                  {data.rtl ? "إضافة عميل" : "Add Client"}
                </button>
              </FormSection>
            </div>
                ...
                {(data.approvals || []).map((approval, index) => (
                  ...
                  >
                    ...
                    <InputField
                      ...
                      value={approval}
                      onChange={(e) =>
                        update(`approvals.${index}`, e.target.value)
                      }
                      placeholder={
                        data.rtl
                          ? "نص الاعتماد أو الشهادة"
                          : "Approval or certification text"
                      }
                      rtl={data.rtl}
                    />
                  </div>
                ))}
                <button onClick={addApproval} className="w-full btn-secondary">
                  {data.rtl ? "إضافة اعتماد" : "Add Approval"}
                </button>
              </FormSection>
            </div>
          </div>
